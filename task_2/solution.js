function calcShipping(sum, min, shipping) {
    ...
    } else if (productSum >= freeShippingMinSum) { // если productsSum Больше или равна freeShippingMinSum,
        shippingSum = 0;
    } else if (productSum > 0 && productSum < freeShippingMinSum) { // если productsSum больше 0 и меньше freeShippingMinSum,
...
function calcDiscount(sum, min, discount) {
    ...
    let discountSum; // создайте переменную discountPrice

    if (productsSum >= discountMinSum) { // если productsSum больше или равно discountMinSum,
        discountSum = discountPart * productsSum / 100; // то присвойте discountPrice значение discountPart процентов от productsSum,
     } else {discountSum = 0; // иначе присвойте discountPrice значения 0
    }
...
function calcInvoice({sum, discountMinSum, discountPart, shippingFreeMinSum, shippingPrice}) {
    ...
    let totalSum = productsSum - discountSum; // создайте переменную totalSum
